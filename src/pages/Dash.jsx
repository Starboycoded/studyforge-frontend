...
export default function Dash({ user, profile, showToast, onTabChange }) {
    ...
    async function loadStats() {
        ...
        try {
            ...
            setStats({ totalCards: cards.length, dueCards: cards.filter(c => !c.due || new Date(c.due) <= new Date()).length, totalSessions: LS.get('sf_sessions', []).length, streak: LS.get('sf_streak', 0) });
            ...
            <div style={{ ...card({ background: `linear-gradient(135deg, ${C.gr}15, ${C.bl}15)`, border: `1px solid ${C.gr}33` }) }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        ...
                        <p style={{ color: C.mu, fontSize: 13, marginTop: 4 }}>{stats?.dueCards > 0 ? `You have ${stats.dueCards} flashcard${stats.dueCards !== 1 ? 's' : ''} due for review.` : 'All caught up! Upload new material to keep learning.'}</p>
                    </div>
                    <button onClick={() => onTabChange(stats?.dueCards > 0 ? 'cards' : 'files')} style={btn('p', { flexShrink: 0 })}>{stats?.dueCards > 0 ? 'Review Now' : 'Upload'}</button>
                </div>
            </div>
        </div>
    );
