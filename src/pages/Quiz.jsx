...
export default function Quiz({ showToast }) {
    ...
    async function generate() {
        ...
        try {
            // FIX: backend /api/quiz expects { content, count } not { topic, numQuestions }
            const result = await apiFetch('/api/quiz', { content: topic.trim(), count: Number(numQ) });
    ...
    // FIX: store selected option INDEX (number) to match backend's correct field (0-3 index)
    ...
    function submit() {
        ...
        questions.forEach((q, i) => {
            // FIX: q.correct is an index (0-3), answers[i] is also an index now
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            {questions.length > 0 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        ...
                        return (
                            <div key={i} style={card()}>
                                    ...
                                    {(q.options || []).map((opt, j) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                disabled={submitted}
                                                style={{ background: bg, border: `1px solid ${border}`, borderRadius: 8, padding: '10px 14px', textAlign: 'left', cursor: submitted ? 'default' : 'pointer', color, fontSize: 13, fontWeight: j === selectedIdx ? 600 : 400, transition: 'all 0.15s' }}
                                            >
                                                {opt}
                                            </button>
                                        );
                                ...
                                {submitted && q.explanation && (
                                    <div style={{ marginTop: 12, background: C.bl + '15', border: `1px solid ${C.bl}33`, borderRadius: 8, padding: '10px 12px', color: C.bl, fontSize: 12 }}>
                                        💡 {q.explanation}
                                    </div>
                                )}
                            </div>
                    ...
                    {!submitted && (
                        <button onClick={submit} disabled={Object.keys(answers).length < questions.length} style={btn('p', { padding: '14px', fontSize: 15 })}>
